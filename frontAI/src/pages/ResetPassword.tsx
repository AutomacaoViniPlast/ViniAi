...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!token) {
      setMessage("Link inválido. Solicite um novo.");
      return;
    }
          ...
          <div className="relative z-10 -mt-16">
            ...
            <div className="ml-12 min-w-[540px] max-w-3xl">
              ...
              <p className="mt-4 text-base leading-relaxed text-muted-foreground lg:ml-2">
                Escolha uma senha forte para proteger seu acesso ao ViniAI.
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="w-full max-w-3xl lg:mr-16 rounded-3xl border-2 border-border/70 bg-card p-7 shadow-[0_20px_40px_hsl(0_0%_0%/0.24)] sm:p-10 relative z-20">
            ...
            <div className="mb-8 space-y-2 text-center">
              ...
              <p className="text-base text-muted-foreground">
                {success ? "Senha redefinida com sucesso." : "Digite e confirme sua nova senha."}
              </p>
            </div>

            {success ? (
              <div className="text-center space-y-4">
                <p className="text-base text-muted-foreground">Você já pode entrar com sua nova senha.</p>
                <button
                  onClick={() => navigate("/auth")}
                  className="w-full rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground transition-all hover:brightness-110"
                >
                  Ir para o login
                </button>
              </div>
              ...
              <>
                ...
                <div className="mt-6 text-center text-base text-muted-foreground">
                  <button onClick={() => navigate("/auth")} className="font-medium text-foreground transition-colors hover:text-primary">
                    Voltar ao login
                  </button>
                </div>
              </>
